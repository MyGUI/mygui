...
namespace MyGUI {
	...
	void Button::onMouseFocus(Window * pWindow, bool bIsFocus) // смена фокуса
	{
		if (pWindow->m_uExData & WES_BUTTON) showFocus(bIsFocus);
	}

	void Button::onMouseButton(Window * pWindow, bool bIsLeftButtonPressed) // нажата левая кнопка мыши
	{
	...
	Button * Button::create(const Coord &Pos, const Coord &Size,
	    Window *parent, const uint16 uAlign, const uint8 uOverlay, const Ogre::String &uSkin)
    {
