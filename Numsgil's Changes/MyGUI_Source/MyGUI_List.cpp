...
namespace MyGUI {
	...
	List::List(__LP_MYGUI_SKIN_INFO lpSkin, uint8 uOverlay, Window *pWindowParent) :
	...
	void List::onKeyButton(Window * pWindow, int keyEvent, char cText) // нажата клавиша
	{
		uint16 oldPos = m_uSelectItem;
		uint16 size = (uint16)m_aString.size();
		if (size == 0) return;

		if (keyEvent == OIS::KC_WEBFORWARD) { // скроллинг
	...
	void List::onKeyFocus(Window * pWindow, bool bIsFocus) // смена фокуса ввода
	{
		if (!bIsFocus) {
			if (pWindowSelect) { // сброс выделения
				pWindowSelect->setState(WS_NORMAL);
	...
	void List::onMouseFocus(Window * pWindow, bool bIsFocus) // смена фокуса
	{
		if (bIsFocus) {
	...
	void List::onMouseClick(Window * pWindow)
	{
		uint16 oldPos = m_uSelectItem;
	...
	void List::onMouseDoubleClick(Window * pWindow) // двойной щелчек мыши
	{
	...
	void List::onOtherEvent(Window * pWindow, uint16 uEvent, uint32 data) // дополнительные события
	{
	...
	void List::size(const Coord &Size) // изменяем размер окна
	{
		...
		while (m_uCurrentFillSize < m_pWindowClient->m_iSizeY) {
			...
			Window * pChild = 
			    m_pWindowClient->spawn<Window>(
			        Coord(0, m_uCurrentFillSize),
			        Coord(m_pWindowClient->m_iSizeX - cutSize,
			                    m_uSizeYButton),
	...
	List * List::addString(const DisplayString & strElement, bool bIsRedraw) // добавить строку в список
	{
	...
	List *List::deleteString(uint16 index, bool bIsRedraw) // удалить строку из списка
	{
		uint16 size = (uint16)m_aString.size();
	...
	List *List::deleteStringAll(bool bIsRedraw) // удаляет все строки
	{
		uint16 size = (uint16)m_aString.size();
		if (size == 0) return this;
	...
	List *List::insertString(uint16 index, const DisplayString & strElement, bool bIsRedraw) // вставить строку в список
	{
	...
	DisplayString List::getString(uint16 index) // возвращает строку
	{
		if (index >= (int16)m_aString.size()) return "";
		return *m_aString[index];
	}

	List *List::setFont(__LP_MYGUI_FONT_INFO lpFont, uint32 colour) // устанавливает шрифт для всего списка
	{
	...
	List * List::create(const Coord &Pos, const Coord &Size,
	    Window *parent, const uint16 uAlign, const uint8 uOverlay, const String &uSkin)
    {
		...
		if (pSkin->data3 != "") { // есть скролл
			String uSkinScroll = pSkin->data3;
			pWindow->m_uSizeXScroll = SkinManager::getSingleton()->getSkin(uSkinScroll)->subSkins[0]->sizeX;
			
			pWindow->m_scroll = pWindow->m_pWindowClient->spawn<VScroll>(
			    Coord(pWindow->m_pWindowClient->m_iSizeX - pWindow->m_uSizeXScroll, 0),
			    Coord(-1, pWindow->m_pWindowClient->m_iSizeY),
		...
		pWindow->m_uSizeYButton = SkinManager::getSingleton()->getSkin(pWindow->m_uSkinButton)->subSkins[0]->sizeY;
		if (pWindow->m_uSizeYButton == 0) pWindow->m_uSizeYButton = 1;
		pWindow->move(Pos.x, Pos.y);
		pWindow->size(Coord(Size.x > 0 ? Size.x : pSkin->subSkins[0]->sizeX,  
