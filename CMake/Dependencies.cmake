...
foreach(_rs IN LISTS MYGUI_BUILD_RENDERSYSTEMS)
	...
	elseif(
		...
		if(_rs EQUAL 8 AND NOT EMSCRIPTEN)
			# GLES on desktop, mostly for tests
			find_path(MYGUI_GLES_INCLUDE_DIR GLES3/gl3.h REQUIRED)
			find_library(MYGUI_GLES_LIBRARY NAMES GLESv2 GLESv3 REQUIRED)
			if(NOT TARGET MyGUI_GLES)
				add_library(MyGUI_GLES UNKNOWN IMPORTED)
				set_target_properties(MyGUI_GLES PROPERTIES
					IMPORTED_LOCATION "${MYGUI_GLES_LIBRARY}"
					INTERFACE_INCLUDE_DIRECTORIES "${MYGUI_GLES_INCLUDE_DIR}")
			endif()
			macro_log_feature(MYGUI_GLES_LIBRARY "gles" "Support for the OpenGL ES render system" "" TRUE "" "")
		else()
	...
	elseif(_rs EQUAL 11)
		if(NOT MYGUI_OSG_FOUND)
			...
			if(unofficial-osg_FOUND)
				...
				find_package(OpenSceneGraph REQUIRED COMPONENTS osg osgDB osgUtil osgGA osgViewer)
				set(MYGUI_OSG_FOUND TRUE)
				set(MYGUI_OSG_INCLUDE_DIR "${OPENSCENEGRAPH_INCLUDE_DIRS}")
				set(MYGUI_OSG_LIBRARIES "${OSG_LIBRARY}" "${OSGDB_LIBRARY}" "${OSGUTIL_LIBRARY}" "${OSGVIEWER_LIBRARY}" "${OSGGA_LIBRARY}" "${OPENTHREADS_LIBRARY}")
				set(MYGUI_OSG_PLATFORM_LIBRARIES "${OSG_LIBRARY}" "${OSGDB_LIBRARY}" "${OPENTHREADS_LIBRARY}")
			endif()
		endif()
