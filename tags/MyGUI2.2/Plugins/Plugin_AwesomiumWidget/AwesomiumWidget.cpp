...
namespace Awesomium
{
	...
	AwesomiumWidget::AwesomiumWidget(MyGUI::WidgetStyle _style, const MyGUI::IntCoord& _coord, MyGUI::Align _align, const MyGUI::WidgetSkinInfoPtr _info, MyGUI::WidgetPtr _parent, MyGUI::ICroppedRectangle * _croppedParent, MyGUI::IWidgetCreator * _creator, const std::string& _name) :
		...
		mOldHeight(getHeight())
	{
		...
		mControl = factory::AwesomiumWidgetFactory::getCore()->createWebView(getWidth(), getHeight(), mIsTransparent, true, 90);
	...
	void AwesomiumWidget::baseChangeWidgetSkin(MyGUI::WidgetSkinInfoPtr _info)
	{
		shutdownWidgetSkin();
	...
	void AwesomiumWidget::initialiseWidgetSkin(MyGUI::WidgetSkinInfoPtr _info)
	{
	}
	...
	void AwesomiumWidget::update(bool _update)
	{
		...
		void* data = lock();
		mControl->render((Ogre::uint8*)data, getTextureRealWidth() * 4, 4, 0);
	...
	{
		mControl->injectMouseWheel(_rel < 0 ? -5 : 5);
