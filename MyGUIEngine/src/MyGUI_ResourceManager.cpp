...
namespace MyGUI
{
	...
	void ResourceManager::loadFromXmlNode(pugi::xml_node _node, std::string_view /*unused*/, Version _version)
	{
		FactoryManager& factory = FactoryManager::getInstance();

		for (auto root : _node.children(mCategoryName.c_str()))
		{
	...
	void ResourceManager::_loadList(pugi::xml_node _node, std::string_view /*unused*/, Version _version)
	{
		for (auto node : _node.children(mXmlListTagName.c_str()))
		{
			std::string source{node.attribute("file").value()};
			if (source.empty())
				continue;
