...
namespace MyGUI
{
	...
	void Tab::updateBar()
	{
		...
		// провер€ем правильность стартового индекса
		if (mStartIndex > 0) {
		...
		// корректируем виджет дл€ пустоты
	...
	void Tab::notifyPressedBarButtonEvent(MyGUI::WidgetPtr _sender, bool _double)
	{
		...
		if (select == mSelectSheet) {
			// стараемс€ показать выделенную кнопку
			showBarSelectButton();
			return;
		}
	...
	void Tab::showBarButton(size_t _index)
	{
		...
		if (_index == mStartIndex) return;
			...
			// уменьшем старт индекс пока не по€витьс€ нужна€
			bool change = false;
	...
	void Tab::_frameEntered(float _frame)
	{
		...
		for (size_t pos=0; pos<mVectorAlphaDataInfo.size(); pos++) {
			...
			// провер€ем нужно ли к чему еще стремитьс€
			if (info.alpha > alpha) {
				alpha += _frame * coef;
				if (info.alpha > alpha) {
