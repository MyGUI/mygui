...
namespace MyGUI
{
	...
	const int SCROLL_VIEW_MOUSE_WHEEL = 50; // колличество пикселей дл€ колеса мыши
	const int SCROLL_VIEW_SCROLL_PAGE = 16; // колличество пикселей дл€ кнопок скрола
	...
	void ScrollView::setTextAlign(Align _align)
	{
		...
		// так как мы сами рулим смещени€ми
		updateView();
	}
			...
			if (mVScroll != null) {
				if (mVScroll->isShow()) {
					...
					if (mHScroll != null) {
						...
						// пересчитываем горизонтальный скрол на предмет скрыти€
			...
			if (mHScroll != null) {
				if (mHScroll->isShow()) {
					...
					if (mVScroll != null) {
						...
						// пересчитываем вертикальный скрол на предмет скрыти€
