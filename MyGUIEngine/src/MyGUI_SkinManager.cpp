...
namespace MyGUI
{
	...
	void SkinManager::_load(xml::ElementPtr _node, const std::string& _file, Version _version)
	{
		...
		while (skin.next())
		{
			...
			else if (skin->getName() == XML_TYPE_PROPERTY)
			{
				const std::string& key = skin->findAttribute("key");
				const std::string& value = skin->findAttribute("value");
				if (key == "Default")
					mDefaultName = value;
			}
		}
