...
namespace MyGUI
{
	...
	void OverlappedLayer::deserialization(pugi::xml_node _node, Version _version)
	{
		...
		{
			setPick(utility::parseBool(_node.attribute("pick").value()));
		}
		else
		{
			setPick(utility::parseBool(_node.attribute("peek").value()));
		}
	}
