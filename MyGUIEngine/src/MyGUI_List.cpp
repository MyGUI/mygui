...
namespace MyGUI
{
	...
	void List::initialiseWidgetSkin(WidgetSkinInfoPtr _info)
	{
		...
		for (VectorWidgetPtr::iterator iter=mWidgetChildSkin.begin(); iter!=mWidgetChildSkin.end(); ++iter)
		{
			...
			else if (*(*iter)->_getInternalData<std::string>() == "Client")
			{
				...
				mWidgetClient->eventMouseButtonReleased = newDelegate(this, &List::notifyMouseReleased);
			}
		}
	...
	void List::notifyMouseReleased(WidgetPtr _sender, int _left, int _top, MouseButton _id)
	{
		...
		if (_sender == mWidgetClient || _sender->isVisible())
		{
			...
			eventListNotifyItem(this, IBNotifyItemData(index, IBNotifyItemData::MouseReleased, _left, _top, _id));
		}
	}
	...
	void List::updateLine(bool _reset)
	{
		...
		if (mOldSize.height < mCoord.height)
		{
			...
			while ( (height <= (mWidgetClient->getHeight() + mHeightLine)) && (mWidgetLines.size() < mItemsInfo.size()) )
			{
				...
				line->eventMouseButtonReleased = newDelegate(this, &List::notifyMouseReleased);
