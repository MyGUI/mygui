...
namespace Export
{
	...
	{
		MYGUIEXPORT void MYGUICALL ExportStaticImage_SetImageRect_rect( MyGUI::Widget* _native,
			Convert<const MyGUI::types::TRect< int > &>::Type _rect )
		{
			static_cast< MyGUI::StaticImage * >(_native)->setImageRect(
				Convert<const MyGUI::types::TRect< int > &>::From( _rect ) );
		}
	}
