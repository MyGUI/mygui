...
void FlashControl::load(const Ogre::String& movieFilename)
{
	...
	const Ogre::String& full_path = MyGUI::helper::getResourcePath(movieFilename);
	if (full_path.empty())
	{
		return;
		//OGRE_EXCEPT(Ogre::Exception::ERR_FILE_NOT_FOUND, "Could not load '" + movieFilename + "', the file was not found.", "FlashControl::load");	
	}

	// работают только абсолютные пути c левыми слешами
	std::string absolute_path = getCurrentWorkingDirectory() + full_path;
...
void FlashControl::bind(const Ogre::UTFString& funcName, FlashDelegate::IDelegate* callback)
{
...
void FlashControl::unbind(const Ogre::UTFString& funcName)
{
...
FlashValue FlashControl::callFunction(Ogre::UTFString funcName, const Arguments& args)
{
	BSTR returnVal = 0;
...
void FlashControl::update()
{
	...
	if (!mIsInitialise)
	{
		if (mOwner)
		{
			uint8* destBuffer = static_cast<uint8*>(mOwner->lock());
