...
namespace MyGUI {
	...
	void VScroll::onMouseMove(MyGUI::Window * pWindow, int16 iPosX, int16 iPosY) // уведомление о движении, но не движение
	{
		...
		// если позици€ изменилась, то отсылаем сообщение
	...
	void VScroll::onMouseClick(MyGUI::Window * pWindow) // нажата и отпущена лева€ кнопка мыши на этом же элементе
	{
	...
	void VScroll::size(int16 iSizeX, int16 iSizeY) // измен€ем размер окна
	{
	        ...
	        Window *parent, uint16 uAlign, uint16 uOverlay, const String &Skin)
	{
		...
		for (uint pos=1; pos<pSkin->subSkins.size(); pos++) {
			...
			if (pSkin->subSkins[pos]->exdata & WES_VSCROLL_TRACK) {
				// элемент €вл€етс€ скролом, прицепл€ем к клиентской области
			...
			if (pChild->m_uExData & WES_CLIENT) pWindow->m_pWindowClient = pChild; // клиентское окно дл€ ползунка
		}
