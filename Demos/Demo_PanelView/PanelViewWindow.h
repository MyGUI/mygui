...
public:
	...
	inline void insertItem(size_t _index, BasePanel * _item) { mPanelView.insertItem(_index, _item); }
	//! Add an item to the end of a list
	inline void addItem(BasePanel * _item) { mPanelView.addItem(_item); }
	//! Get item from specified position
	inline BasePanel * getItem(size_t _index) { return mPanelView.getItem(_index); }
	//! Search item, returns the position of the first occurrence in list or ITEM_NONE if item not found
	inline size_t findItem(BasePanel * _item) { return mPanelView.findItem(_item); }

	//! Remove item at a specified position
	inline void removeItemAt(size_t _index) { mPanelView.removeItemAt(_index); }
	//! Remove item at a specified position
	inline void removeItem(BasePanel * _item) { mPanelView.removeItem(_item); }
	//! Remove all items
