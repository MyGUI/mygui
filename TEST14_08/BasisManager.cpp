...
//	if (mGUI) { // дл€ смены из полноэкранного и обратно
...
//	if (!mRoot->restoreConfig()) { // сведени€ об настройках отсутствуют
//		saveDefaultConfig(); // пытаемс€ сохранить дефолтный вариант
...
void BasisManager::saveDefaultConfig() // сохран€ет дефолтные настройки в файл
{
	GameConfigFile config;

	RenderSystemList * rList = mRoot->getAvailableRenderers();
	// перебераем все дефолтные значени€ и сохран€ем их в файл
...
void BasisManager::changeState(BasisState* state, bool bIsFade)
{
	...
	if ( !mStates.empty() ) {
		if (bIsFade) { // потом вызовем но уже после затенени€
			mFadeState = state;
	...
	mGUI->m_pEventCallback = state; // текущий класс дл€ передачи событий
...
void BasisManager::popState(bool bIsFade)
{
	// cleanup the current state
	if ( !mStates.empty() ) {
		mGUI->m_pEventCallback = 0; // текущий класс дл€ передачи событий
		mStates.back()->exit();
		mStates.pop_back();
	}
	// resume previous state
	if ( !mStates.empty() ) {
		mGUI->m_pEventCallback = mStates.back(); // текущий класс дл€ передачи событий
		mStates.back()->resume();
...
//=======================================================================================
OIS::MouseState BasisManager::g_MouseState; // состо€ние мыши дл€ оконного режима
OIS::MouseEvent BasisManager::g_MouseEvent(0, g_MouseState); // аргументы мыши дл€ оконного режима
//=======================================================================================
void BasisManager::windowEventMouseMove(RenderWindow* rw, LPARAM position) // позици€ мыши
{
